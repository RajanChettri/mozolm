...
class NGramStateCache {
 public:
  ...
  explicit NGramStateCache(int state) : state_(state) { last_accessed_ = -1; }

  // Updates cache information word initially, i.e., with no prefix.
  void SetCache(int access_counter, const std::vector<double>& arc_weights);
...
class NGramWordFstModel : public NGramFstModel {
 ...
 private:
  ...
  int NextCompleteState(int state, int model_state, int prefix_length);
  ...
  std::vector<NGramStateCache> state_cache_;  // Cache for state information.
