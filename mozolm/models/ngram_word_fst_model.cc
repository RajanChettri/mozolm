...
absl::Status NGramWordFstModel::GetNewCacheIndex(StdArc::StateId s) {
  if (state_cache_.size() < max_cache_size_) {
    cache_index_[s] = state_cache_.size();
    state_cache_.push_back(NGramStateCache(s));
  } else {
    ...
    state_cache_[index_to_update] = NGramStateCache(s);
...
absl::Status NGramWordFstModel::EnsureCacheIndex(int state) {
  ...
  absl::Status status = GetNewCacheIndex(state);
  if (status == absl::OkStatus()) {
    state_cache_[cache_index_[state]].SetCache(cache_accessed_++, weights);
  }
  return status;
}
...
bool NGramWordFstModel::ExtractLMScores(int state, LMScores *response) {
  ...
  if (!model_state.ok()) {
    // No model state associated with this implicit state.
    return false;
  }
  state_cache_[cache_index_[*model_state]].set_last_accessed(cache_accessed_++);
...
void NGramStateCache::SetCache(int access_counter,
